...
void PcapSource::GetDeviceName(int index, TCHAR *buf, int cchLen)
{
	...
	//yhctest Utils::AnsiToUtf16(name, tName, 256);
...
bool PcapSource::Reconnect(int index)
{
    ...
    // Reconnect
    bool b = Initialize();
	//yhctest Utils::DbgPrint(TEXT("Initialize returns %s\n"), b ? TEXT("true") : TEXT("false"));
    ...
    for (int i = 0; i < 20; i++)
    {
        Sleep(1000);
        n = EnumDevices();
		//yhctest Utils::DbgPrint(TEXT("EnumDevices returns %d\n"), n);
        if (n > 0)
            break;
    }
    ...
    if (n > 0)
    {
		...
		//yhctest Utils::DbgPrint(TEXT("SelectDevice returns %d\n"), result);
        return true;
    }
    else
    {
